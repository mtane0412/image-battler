...
4. **たたかう**: ホームでステージ(未選択ならデフォルトステージ)とバトル形式(1vs1 / 2vs2 / バトルロイヤル)を選び、カードを選んで「バトルスタート」。1vs1 は2枚、2vs2 は4枚(選んだ順に 1P チーム → 2P チーム)、バトルロイヤルは3〜5枚(選んだ順にエントリー)です。バトル展開は JavaScript が計算し、実況セリフを Gemini Nano がリアルタイム生成します
